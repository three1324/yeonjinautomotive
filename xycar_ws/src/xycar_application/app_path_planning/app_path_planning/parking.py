...
# `node`를 외부에서 주입받도록 
#=============================================
def init_parking_module(): #external_node):
    # simulator.py에서 만든 node를 parking.py에서 사용하도록 설정
    global node, motor_pub
    
    node = rclpy.create_node('Driver')
    
    if motor_pub is not None:
        print("⚠ Warning: init_parking_module() called multiple times!")
        return  # 이미 설정되어 있으면 다시 실행하지 않음
   
    #node = external_node  # 외부에서 받은 노드를 사용
    motor_pub = node.create_publisher(XycarMotor, 'xycar_motor', 10)
...
def drive(angle, speed):
    xycar_msg.angle = float(angle)
    xycar_msg.speed = float(speed)
