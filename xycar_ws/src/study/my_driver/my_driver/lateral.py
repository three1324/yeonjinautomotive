...
class OvertakeBehavior:
    ...
    규칙이 **두 줄**이다 (2026-08-21 사용자 결정으로 단순화):
      ...
      복귀 : 차량이 안 보이는 상태가 lost_hold_sec(2초) 이어졌다
    ...
    def __init__(self, shift_px,
        ...
        # 0 이면 차가 계속 보이는 한 계속 벌린 채로 달린다 — 앞차를 따라가는
        # 상황에는 그게 맞다. 다만 주차된 차나 트랙 밖 차가 시야에 계속
        # 걸리면 한쪽에 붙은 채 도는 위험이 남는다. 그때만 켜서 쓴다.
