...
def extract(result, names, width, height, dashed_conf, solid_conf,
    ...
    for box in result.boxes:
        ...
        if cls == "traffic_light":
            ...
            # 이전에는 conf 가 가장 높은 차를 골랐는데, conf 는 거리와 무관하다 —
            # 멀리 있는 차가 또렷하게 찍히면 그쪽이 뽑혀서, 바로 앞의 차를 두고
            # 먼 차 기준으로 회피 방향을 정하는 일이 생긴다.
            # bbox 하단 y 는 거리의 단조 함수라(모듈 docstring) y2 가 클수록 가깝다.
