...
def run(drop_rate=0.0, noise=0.0, seed=1, verbose=False, lateral=0.54):
    ...
    return {
        'frames': frames, 'flips': flips, 'no_path': no_path,
        'center_err': center_err, 'target_err': target_err,
        'naive_delta': naive_delta, 'steer_gap': steer_gap,
    }


def _plan(left, right):
    """rubbercone_node._plan_path() 와 같은 결정 순서를 재현한다.

    [2026-08-22] 편측 폴백을 없앤 뒤의 버전이다 — 양벽 중심선이 안 나오면
    그냥 빈 경로. resolve_side/corridor_side/offset_from_single_wall 은
    아직 geometry.py 에 남아 있지만(nearest_pair 롤백용) 이 함수는 더 이상
    쓰지 않는다.
    """
    if left and right:
...
def main():
    ...
    for name, kw in (
        ...
        r = run(**kw)
        print('\n[%s]' % name)
        print('  프레임 %d | 사슬혼입 %d | 경로없음 %d'
              % (r['frames'], r['flips'], r['no_path']))
