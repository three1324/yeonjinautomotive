...
코스 형상 (2026-08-22 확인) — 파라미터의 근거
  ...
  [2026-08-22] 0.85 -> 1.10(UNDER_VOLTAGE 완화) -> 0.85 -> 1.10 -> **0.90**.
  angle_limit 은 35 그대로다(기계적 포화점) — 이 값은 그와 별개로 조정한
  것이다. 0.90 은 1.10 과 0.85 사이 절충값 — R=0.5m 급커브의 코너 컷
  여유(0.20m)를 거의 다 쓴다. 그 전 이력: 급커브에서 못 꺾고 안쪽 콘을
  쳐서 0.85 -> 0.80 으로 줄여봤으나, 짧게 볼수록 직선 응답이 나빠지고
  같은 건에서 고친 목표점 clamp 순서 버그(scan_callback 의 ★ 주석) 쪽
  영향이 훨씬 커서 0.85 로 되돌렸었다.
