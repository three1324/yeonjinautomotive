...
전제가 되는 실측 치수 (2026-08-22 확정)
    ...
    같은 줄 콘 중심 간격            P = 0.425 m   (실측 2026-08-22: 한 칸 건너 0.85)
...
반대로 튄다. 그래서 좌우를 **콘 사이 거리(사슬)** 로 나눈다:
    ...
    콘 간격 0.425  <  사슬 문턱 0.60  <  복도 폭 0.80
...
CONE_PITCH_M = 0.425      # 실측: 한 칸 건너 두 콘 사이 0.85 -> P = 0.425
...
def polyline_closest_point(poly, p, extend=0.0):
    ...
    으로부터 P*2 = 0.85m 로 잡혀, 반대편 벽(0.80m)보다 **멀어진다** —
...
SIDE_MIN_SIN = 0.55
# 직전 목표점(시간 연속성)을 **뒤집을 수 있는** 수직 판정 확신도.
# 이보다 뚜렷하면 기하 증거가 연속성을 이긴다 — 아래 resolve_side 참고.
#
# 값의 근거 (2026-08-22, 오염 회복 시뮬 + synth_check 회귀로 확정):
#   0.34~0.46  수직 판정이 **자신 있게 틀리는** 구간 (합성검증 실측)
#   0.55       SIDE_MIN_SIN — "근거가 없을 때 혼자 결정해도 되는" 문턱
#   0.65       여기. 상충하는 증거를 **이기려면** 그보다 뚜렷해야 한다
#   0.80       너무 엄격 — conf 0.74 인 오염 사례에서 회복 실패했다
# 0.65/0.55 는 회복 결과가 같고 synth_check 수치도 동일했다. 둘 중
# 보수적인(연속성을 덜 깨는) 0.65 를 고른다.
SIDE_OVERRIDE_SIN = 0.65
...
def build_chains(cones, chain_max_dist, extend, reassign_dist, min_cones,
    ...
    ※ "가장 가까운 콘이 있는 사슬에 붙인다"로는 안 된다. 콘 하나가 빠지면
      자기 벽까지 0.85m, 반대 벽까지 0.80m 라 **반대 벽이 이긴다.**
      반드시 방향(폴리라인)을 봐야 한다.
    """
    pool = list(cones)
    if not pool:
...
def chain_tangent(chain, i):
    """사슬 위 i 번째 콘에서의 **국소 접선 방향**(단위벡터). 없으면 None.

    양옆 이웃을 쓰고(끝점이면 인접 세그먼트 하나), 사슬이 1개면 방향을
    정의할 수 없으므로 None 이다.
    """
    n = len(chain)
    if n < 2:
    ...
    j = max(0, i - 1)
    k = min(n - 1, i + 1)
    dx = chain[k][0] - chain[j][0]
    dy = chain[k][1] - chain[j][1]
...
def centerline(left, right, min_gap, max_gap, extend, max_tangent_cos=0.5):
    ...
    ★ [2026-08-22] 폭 창만으로는 **같은 벽 콘끼리 짝지어지는 것을 못 막는다.**
      실패 사례(재현됨): 커브에서 한쪽 벽 콘 5개가 전부 ROI 안에 있고 먼
      쪽 콘의 y 부호가 넘어가면, build_chains 의 시드가 그 벽을 두 사슬로
      쪼갠다. 그러면 아래 두 값이 **둘 다 폭 창 안**이라 통과해 버린다:

          같은 줄 한 칸 건너   P*2 = 0.85 m    (실측 2026-08-22)
          커브에서 접선 이탈    s^2/(2R) — s=1.3, R=0.9 이면 0.94 m

      ★ P*2(0.85) 와 복도 폭 W(0.80) 의 차이가 **5cm 뿐이다.** 콘 중심
        추정 오차(라이다 점 2~3개, ±3cm)와 같은 자릿수라, 거리 문턱으로는
        원리적으로 못 가른다. max_gap 은 보조일 뿐 **이 방향 검사가 본체다.**

      결과는 **벽 위에 얹힌 가짜 중심선**이고, 차는 같은 줄 두 콘 사이로
      나가려 든다. 실제 증상이었다.

      거리로는 못 가르므로 **방향**으로 가른다. 진짜 복도 대응은 벽에
      거의 **수직**이고(두 벽이 동심이라 대응 방향이 곧 법선), 같은 벽
      대응은 접선과 거의 **나란하다**(위 사례에서 cos = 0.97).
      max_tangent_cos 는 그 |cos| 의 상한이다:

          0.5  = 접선과 60도 이상 벌어진 대응만 통과 (기본값)
          1.0  = 이 검사를 끈다 (옛 동작)

      접선을 못 구하면(사슬 길이 1) 검사를 건너뛴다 — 그 경우는 애초에
      min_cones 로 걸러진다.
    """
    mids = []
    for src, dst in ((left, right), (right, left)):
        for i, c in enumerate(src):
            ...
            if q is None or not (min_gap <= d <= max_gap):
                continue
            t = chain_tangent(src, i)
            if t is not None and max_tangent_cos < 1.0 and d > 1e-9:
                vx, vy = (q[0] - c[0]) / d, (q[1] - c[1]) / d
                if abs(vx * t[0] + vy * t[1]) > max_tangent_cos:
                    continue          # 벽을 따라 난 대응 = 같은 벽이다
...
def resolve_side(chain, offset, conf_side, conf, prev_target):
    ...
    ── ★ 자기강화 루프와 그 차단 (2026-08-22) ──
    위 규칙만 두면 **한 번 틀리면 못 빠져나온다.** 틀린 목표점이 나오면
    호출자가 그걸 _prev_target 에 저장하고, 다음 프레임에서 그 값이 다시
    1순위로 같은 쪽을 고르기 때문이다. 경로가 계속 나오는 한 _prev_target
    은 폐기되지도 않는다(호출자는 **경로를 못 냈을 때만** 나이를 먹인다).
    옛 주석은 "양벽 프레임이 자기교정한다"고 했지만, 커브에서 편측 폴백이
    이어지면 그 프레임이 안 온다 — 실차에서 잘못 문 채 계속 가는 증상.

    그래서 **기하 증거가 아주 뚜렷하면(conf >= SIDE_OVERRIDE_SIN) 연속성을
    깬다.** 근거: prev_target 은 어디까지나 사전확률이고, 수직 판정은 직접
    측정이다. 0.65 는 이 판정이 무너지는 방사 방향 구간(0.34~0.46)과 충분히
    떨어져 있어, 그 구간의 "자신 있게 틀림"에 걸리지 않는다.
    (SIDE_MIN_SIN 0.55 로 뒤집지 않는 이유: 그 값은 "근거가 아예 없을 때
     혼자 결정해도 되는" 문턱이지, 상충하는 증거를 이길 문턱은 아니다.)

    검증: 목표점이 벽 반대쪽으로 오염된 상태에서 6프레임을 돌리면
      옛 동작   [+1,+1,+1,+1,+1,+1]  영영 못 빠져나온다
      수정 후   [-1,-1,-1,-1,-1,-1]  첫 프레임에 회복
    synth_check 수치는 옛 동작과 완전히 동일하다(회귀 없음).

    양쪽 벽이 보이는 프레임의 중심선은 side 를 아예 쓰지 않고, 콘을 오래
    놓치면 호출자가 prev_target 을 버린다 — 그 두 탈출구는 그대로 있다.
    ...
    if prev_target is not None:
        ...
        for cand in (1, -1):
            ...
            if d < best_d:
                best_d = d
                best = cand
        if best != conf_side and conf >= SIDE_OVERRIDE_SIN:
            # 연속성과 기하가 **상충**하는데 기하가 아주 뚜렷하다.
            # 직전 목표가 틀렸다고 보고 여기서 루프를 끊는다.
            return conf_side
        return best
