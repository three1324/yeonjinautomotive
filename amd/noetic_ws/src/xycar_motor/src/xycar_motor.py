...
class motor:
    ...
    def VescStateCallback(self, data):
        ...
        if (self.Battery < 5) and ((time.time()-self.start_time) > 1):
            rospy.logwarn("The current battery voltage is lower than 8V.")
            self.start_time = time.time()

    def ros_init(self):
        rospy.init_node('xycar_motor')
        #rospy.Subscriber("xycar_motor", XycarMotor, self.callback, queue_size=1)
        rospy.Subscriber("xycar_motor", Float32MultiArray, self.callback, queue_size=1)

        self.angle_offset = 0 #rospy.get_param("~angle_offset")
        self.motor_type = 0 #rospy.get_param("~motor_type")
    ...
    def callback(self, data):
        ...
        angle = max(-50, min(data.data[0], 100))
        speed = max(-50, min(data.data[1], 100))
        ...
        Angle = (((float(angle + self.angle_offset) + self.angle_bias_1) * self.angle_weight)) + self.angle_bias_2
        Speed = ((float(speed) + self.speed_bias_1) * self.speed_weight) + self.speed_bias_2
